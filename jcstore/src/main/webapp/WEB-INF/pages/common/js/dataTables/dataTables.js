...
(/** @lends <global> */function( $ ) {
	...
	{
		//daijinma 判断是否请求url改变，判断跳页用，
		if(window.DataTableUrl == oSettings.sAjaxSource){
			
		}else{
//			window.localPage = 1;
		}
		//daijinma 判断是否请求url改变，判断跳页用，end
	...
	function _fnAjaxUpdateDraw ( settings, json )
	{
		...
		for ( var i=0, ien=data.length ; i<ien ; i++ ) {
			//by caicai
			var myData = {};
			$.map(data[i],function (mitem,midx){
				var rMyData = {};
				var myItem = "";
				if(typeof mitem == "string"){
					var arrEntities = {'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;','\n':'<br/>','\r':'<br/>','\r\n':'<br/>'};
					myItem = mitem.replace(/(<|>|&|\"|\n|\r|\r\n)/igm,function(all,t){
