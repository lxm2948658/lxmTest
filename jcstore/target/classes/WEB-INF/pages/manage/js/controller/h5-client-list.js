...
define(function(require, exports, module) {
	...
	var belong=filter.getQueryString("uid");
	...
	window.clientList = new listTable({
		'el':'#messageList',
		'url':domainUrl+'/manage/scene/findScene?belong='+belong,
	...
	var addDialog = dialog.dialog({
		...
		button:{
			"配置场景":function(){
				...
				client.addh5client(id,belong,title,picture,url).ready(function(data){
					if(data.status=="1"){
						dialog.lite("添加成功");
						clientList.draw();
...
//	删除dialog
	var deletDialog = dialog.dialog({
		content:'<p style="text-align: center; color: #f00; font-size: 16px;margin-top: 10px;">确认删除该条场景？</p>',
		title:"系统提示",
		width:378,
		button:{
			"确定":function(){
				client.delh5client({id:_id}).ready(function(data){
					if(data.status=="1"){
						deletDialog.close();
						clientList.draw();
					}else{
